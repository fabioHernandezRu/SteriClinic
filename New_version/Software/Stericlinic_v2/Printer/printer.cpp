...
void printer::print(char * document) {
	FILE *fp;
	char path[1035];
	snprintf(buffer_print, sizeof(buffer_print),
			"python /Stericlinic/Printer/Python/print.py %s %s",
			this->port.c_str(), document);
...
int printer::printF(char *str, size_t count, const char *fmt, ...) {
	size_t ret;
	va_list ap;
